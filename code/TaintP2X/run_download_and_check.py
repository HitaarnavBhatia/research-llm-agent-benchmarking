...
def run_pysa_check(folder):
    """对指定文件夹运行 pysa 检查"""
    # folder is absolute path to the cloned repo.
    # .pyre_configuration must be in the cwd where we run pyre.
    # We run pyre from the root TaintP2X directory, so write config there.
    abs_folder = os.path.abspath(folder)
    config = {
        "site_package_search_strategy": "pep561",
        "source_directories": [abs_folder],
        "taint_models_path": [
            os.path.abspath("./Taint_Propagation/taint"),
            f"{abs_folder}/source"
        ],
        "search_path": [os.path.abspath("./Taint_Propagation/stubs")],
    }
    
    # Write config to project root (cwd where pyre will run)
    config_path = "./.pyre_configuration"
    with open(config_path, "w") as f:
        json.dump(config, f, indent=2)
    
    folder_name = os.path.basename(folder)
    output_dir = os.path.abspath(f"{folder}/pysa-runs_{folder_name}")
    
    # 运行检查，添加120分钟超时限制
    command = f"pyre analyze --no-verify --save-results-to {output_dir}"
    try:
        result = subprocess.run(command, shell=True, cwd=".", timeout=7200)  # 7200秒 = 120分钟
    except subprocess.TimeoutExpired:
        print(f"检查超时（超过120分钟），跳过项目：{folder_name}")
...
def process_github_repo(repo, download_dir, commit_hash=None, version=None, max_retries=3):
    """下载并检查GitHub仓库"""
    start_time = time.time()  # 记录开始时间
    timeout = 10800  # 180分钟超时(10800秒) - Increased for massive repos
    ...
    if version:
        unique_dir_name += f"_{version}"
    
    # Include the first 7 characters of the commit hash to distinguish versions
    if commit_hash:
        unique_dir_name += f"_{commit_hash[:7]}"
    
    target_dir = os.path.join(download_dir, unique_dir_name)
    
    # Use unique_dir_name as the key in checked_repos.json so versions are tracked separately
    repo_key = unique_dir_name
    ...
    for attempt in range(max_retries):
        try:
            ...
            if time.time() - start_time > timeout:
                print(f"总时间超过180分钟，跳过仓库: {full_repo_name}")
                return False
            ...
            # Checkout the specific commit hash if provided
            if commit_hash:
            ...
            # 如果没有问题，删除结果
            if not has_issue:
                print(f"未发现问题，保留仓库以供检查: {target_dir}")
                # shutil.rmtree(target_dir) # commented out to prevent deletion
...
def read_repos_from_json(file_path):
    """从 JSON 文件读取仓库列表，返回 (url, commit_hash, version) 元组列表"""
    with open(file_path, "r") as f:
        data = json.load(f)
        repos = []  # Changed to a list to allow multiple versions of the same repo
        for repo_info in data:
            repo_url = repo_info.get("url")
            commit_hash = repo_info.get("commit_hash")
            version = repo_info.get("version")
            if repo_url:
                repos.append((repo_url, commit_hash, version))
    return repos

# 添加线程锁用于同步文件写入
file_lock = threading.Lock()

def process_github_repo_with_lock(repo_info, download_dir, max_retries=3):
    """带有文件锁的仓库处理函数"""
    repo_url, commit_hash, version = repo_info
    try:
        return process_github_repo(repo_url, download_dir, commit_hash, version, max_retries)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务并获取Future对象
        future_to_repo = {
            executor.submit(process_github_repo_with_lock, repo_info, download_dir): repo_info 
        ...
        for future in future_to_repo:
            ...
            try:
                ...
                print(f"处理仓库 {repo_url} 时发生异常: {str(e)}")
                failed_repos.append(repo_url)
